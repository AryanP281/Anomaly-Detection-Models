...
def variance(X, mu) :
    """Calculates the variances for the features.
    X = a NumPy matrix containing the data [ no. of training inputs X no. of features per example ]
    mu = a NumPy matrix containing the means for the features [ 1 X no. of features per example ]"""

    sigma = (np.subtract(X, mu))**2
    sigma = np.sum(sigma, axis=0) / X.shape[0]

    return sigma

def return_gaussian_value(X, mu, sigma) :
    ...
    denominator = 1 / (((2*np.pi)**0.5) * (sigma**0.5))

    return numerator * denominator

#********************Classes*****************
class UnivariateGaussianModel(object) :
    def __init__(self) :
    ...
    def calculate_parameters(self, X) :
        ...
        self.parameters.append(variance(X, self.parameters[0]))
        self.parameters[1].shape = (1, self.parameters[1].shape[0])
    ...
    def pdf(self, X) :
        ...
        #Getting the feature-wise probabilities for the training inputs
        feature_probabilities = return_gaussian_value(X, self.parameters[0], self.parameters[1]) 

        #Calculating the probabilities for each training input
        inputs_probabilities = np.prod(feature_probabilities, axis=1)

        return inputs_probabilities
